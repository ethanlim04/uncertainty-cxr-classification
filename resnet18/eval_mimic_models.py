# Evaluate Bayesian ResNet model and Probabilistic model with MIMIC III dataset
...
val_x, val_y = prepare_dataset(pd.read_csv('mimic_test.csv'))
...
probabilistic_checkpoint = torch.load("/media/ethan/model_saves/probabilistic_mimic/_best.pth.tar", map_location=device)
bayesian_checkpoint = torch.load("/media/ethan/model_saves/bayesian2_mimic/_best.pth.tar", map_location=device)
