# Evaluate Bayesian DenseNet model with MIMIC dataset
...
def prepare_dataset(dataframe, datapoints):
    x = dataframe['img_paths'].tolist()
    y = dataframe['labels'].tolist()
    return x[datapoints:], y[datapoints:]
...
datapoints = len(pd.read_csv('../cxr/mimic3.csv') * 9/10)
val_x, val_y = prepare_dataset(pd.read_csv('../cxr/mimic3.csv'), datapoints)
...
checkpoint_path = "/media/ethan/model_saves/densenet_512/0.7912636399269104_20_54600.ckpt"
